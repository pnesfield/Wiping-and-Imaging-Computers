...
int main(int argc, char* argv[])
{
    ...
    DISK_GEOMETRY pdgeom = { 0 };  // disk drive geometry structure
    ULONGLONG DiskSize = 0;    // size of the drive, in bytes
    BOOL bResult = FALSE;                 // results flag
    DWORD junk = 0;                     // discard results
    bResult = DeviceIoControl(device,                       // device to be queried
        IOCTL_DISK_GET_DRIVE_GEOMETRY, // operation to perform
        NULL, 0,                       // no input buffer
        &pdgeom, sizeof(pdgeom),            // output buffer
        &junk,                         // # bytes returned
        (LPOVERLAPPED)NULL);          // synchronous I/O
    printf("Bytes per sector    = %ld\n", (ULONG)pdgeom.BytesPerSector);
    printf("Sectors/track       = %ld\n", (ULONG)pdgeom.SectorsPerTrack);
    printf("Tracks per Cylinder = %ld\n", (ULONG)pdgeom.TracksPerCylinder);
    printf("Number of Cylinders = %li\n", pdgeom.Cylinders);
    //printf("Disk Size = %li\n", pdgeom.DiskSize);
